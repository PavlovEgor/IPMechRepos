...
def plot_time_flame_coordinate(x, y, left_t_lim=0, right_t_lim=np.infty):
    ...
    plt.title(
        f'Зависимость положения пламени от времени по Qdot \n {a} cm/s')
...
def plot_time_flame_speed(x, y, left_t_lim=0, right_t_lim=np.infty):
    ...
    plt.title(f'Зависимость скорости пламени от времени по Qdot \n средняя скорость {np.mean(ny)}')
